...
#include <ilqgames/utils/post_processing_narrow_door.h>
...
#include <kalman/narrow_2_agent.hpp>
...
ilqgames::Narrow2AgentConfig config;
std::shared_ptr<ilqgames::Narrow2Agent> problem;
...
int main(int argc, char** argv) {
  ...
  config.width = nh.param<float>("width", 0.0);
  config.start_x = nh.param<float>("start_x", 0.0);
  config.end_x = nh.param<float>("end_x", 0.0);
  config.upper_y = nh.param<float>("upper_y", 0.0);
  config.lower_y = nh.param<float>("lower_y", 0.0);
  config.wObstacle = nh.param<float>("wObstacle", 0.0);
  
  // Set up the game.
  problem = std::make_shared<ilqgames::Narrow2Agent>();
  ...
  while (ros::ok()){
    ...
    if (run_solver){       
      ...
      problem = std::make_shared<ilqgames::Narrow2Agent>(); // need to reinitialize
