#ifndef ILQGAMES_EXAMPLES_NARROW_2_AGENT_H
#define ILQGAMES_EXAMPLES_NARROW_2_AGENT_H
...
struct Narrow2AgentConfig{
  float x1_goal, y1_goal, x2_goal, y2_goal, v1_limit, v2_limit, 
        prox1_threshold, prox2_threshold, wGoal, wSpeed, wProximity, wControl,
        width, start_x, end_x, upper_y, lower_y, wObstacle;
};

class Narrow2Agent : public TopDownRenderableProblem {
 public:
  ~Narrow2Agent() {}
  Narrow2Agent() : TopDownRenderableProblem() {}

  void SetupConfig(const Narrow2AgentConfig& config){
