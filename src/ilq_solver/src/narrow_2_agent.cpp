...
#include <ilqgames/ros/narrow_2_agent.h>
...
void Narrow2Agent::ConstructDynamics() {
  dynamics_.reset(new ConcatenatedDynamicalSystem(
      {std::make_shared<Dyn>(), 
       std::make_shared<Dyn>()}));
}

void Narrow2Agent::ConstructInitialState() {
...
void Narrow2Agent::ConstructPlayerCosts() {
  ...
  // 5. obstacle cost
  const Polyline2 lane1({Point2(this->start_x, this->upper_y), Point2(this->end_x, this->upper_y)}); // upper wall
  const Polyline2 lane2({Point2(this->start_x, this->lower_y), Point2(this->end_x, this->lower_y)}); // lower wall
  const std::shared_ptr<SemiquadraticPolyline2Cost> p1_upper_r_cost(
      new SemiquadraticPolyline2Cost(this->wObstacle, lane1,
                                     {kP1PxIdx, kP1PyIdx}, this->width,
                                     true, "UpperWallRightBoundary"));
  const std::shared_ptr<SemiquadraticPolyline2Cost> p2_lower_l_cost(
      new SemiquadraticPolyline2Cost(this->wObstacle, lane2,
                                     {kP2PxIdx, kP2PyIdx}, -this->width,
                                     false, "LowerWallLeftBoundary"));
  
  p1_cost.AddStateCost(p1_upper_r_cost);
  p2_cost.AddStateCost(p2_lower_l_cost);
}

inline std::vector<float> Narrow2Agent::Xs(const VectorXf& x) const {
  return {x(kP1PxIdx), x(kP2PxIdx)};
}

inline std::vector<float> Narrow2Agent::Ys(const VectorXf& x) const {
  return {x(kP1PyIdx), x(kP2PyIdx)};
}

inline std::vector<float> Narrow2Agent::Thetas(
