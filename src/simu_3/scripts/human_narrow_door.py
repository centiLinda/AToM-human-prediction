...
def generate_trajectory(x_start, y_start, x_goal, y_goal, detour_strength, velocity, num_steps=11):
    ...
    sigma = 2 # std deviation, controls shape of the overall distribution, 5
...
def talker():
    ...
    x_start = rospy.get_param('/x1_start', 0.0)
    y_start = rospy.get_param('/y1_start', 6.0)
    x_goal = rospy.get_param('/x1_goal', 10.0)
    y_goal = rospy.get_param('/y1_goal', 6.0)
    detour = rospy.get_param('/human_detour', 0.0)
    velocity = rospy.get_param('/human_v', 1.0)

    rospy.set_param('/moving', True)
    ...
    while not rospy.is_shutdown():
        ...
        moving = rospy.get_param('/moving')
        if trigger and not trigger_processed:
            if not moving:
                point_msg = Point4D()
                point_msg.header.stamp = rospy.Time.now()
                point_msg.x, point_msg.y = previous_pos
                point_msg.vx = 0
                point_msg.vy = 0
