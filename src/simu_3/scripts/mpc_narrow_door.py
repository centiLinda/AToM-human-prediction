...
def discrete_mpc(traj_h):
    ...
    set_size = 100      # size of discrete trajectory set
    v_pref = 0.6        # preferred robot speed
    ...
    while stepCount <= (N - rollout_step): # planning horizon is 10-6+1=5
        ...
        # Lower wall: penalize robot if it gets within buffer of the lower wall (y <= 1.75 + buffer) between x = 2 - buffer and x = 3 + buffer
        c_lower_wall = np.zeros_like(traj_set[:, :, 0])
        for i in range(traj_set.shape[0]):  # iterate over each trajectory set
            within_x_range = (traj_set[i, :, 0] >= (2.0 - buffer)) & (traj_set[i, :, 0] <= (3.0 + buffer/2.0))
            below_lower_wall = traj_set[i, :, 1] <= (1.75 + buffer)
            # Penalize based on both x and y proximity to the lower wall, considering buffer
            x_penalty = np.maximum(0, traj_set[i, within_x_range & below_lower_wall, 0] - (2.0 - buffer)) ** 2
            y_penalty = np.maximum(0, (1.75 + buffer) - traj_set[i, within_x_range & below_lower_wall, 1]) ** 2
            c_lower_wall[i, within_x_range & below_lower_wall] = x_penalty + y_penalty

        c_obstacle = c_lower_wall
        c_obstacle = np.sum(c_obstacle, axis=-1)

        #########
        # Total #
        #########
        c_total = cost_weights['goal'] * c_goal + cost_weights['human_avoid'] * c_human_avoid + cost_weights['obs_avoid'] * c_obstacle
