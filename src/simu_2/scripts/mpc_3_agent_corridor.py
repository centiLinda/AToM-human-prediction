...
import rospy
from helpers.msg import Point4D, Point4DArray, Point4DThreeArray
import numpy as np
...
def discrete_mpc(traj_h_both):
    ...
    while stepCount <= (N - rollout_step): # planning horizon is 10-6+1=5
        ...
        c_goal = c_goal ** 2
        c_total = cost_weights['goal'] * c_goal
        
        #####################
        # 2-Human avoidance #
        #####################
        for traj_h in traj_h_both:
            if traj_h is not None:
                ...
                c_human_avoid = np.sum(c_human_avoid, axis=-1) ** 2
                c_total += cost_weights['human_avoid'] * c_human_avoid

        ########################
        # 3-Obstacle avoidance #
        ########################
        # 2 walls
        c_left = np.minimum(0, traj_set[:, :, 0] - 1.0) ** 2
        c_right = np.maximum(0, traj_set[:, :, 0] - 3.0) ** 2
        c_obstacle = c_left + c_right
        c_obstacle = np.sum(c_obstacle, axis=-1)
        c_total += cost_weights['obs_avoid'] * c_obstacle
...
def predicted_position_callback(msg):
    ...
    latest_human_pred = (msg.agent1_traj, msg.agent2_traj)
    received_pred = True
...
if __name__ == '__main__':
    try:
        ...
        rospy.Subscriber('/game_results', Point4DThreeArray, predicted_position_callback)

        goal[0] = rospy.get_param('/x3_goal')
        goal[1] = rospy.get_param('/y3_goal')
