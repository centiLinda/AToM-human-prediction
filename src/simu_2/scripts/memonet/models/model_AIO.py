...
#######################################################################
# this file is not used, only referenced by the locally-trained model #
#######################################################################
...
class model_encdec(nn.Module):
    ...
    def get_trajectory(self, past, abs_past, seq_start_end, end_pose):
        ...
        for i in range(20):
            ...
            prediction_single = torch.zeros((batch_size, self.future_len - 1, 2)).cuda()
            ...
            for i_frame in range(1, self.future_len):
                prediction_single[:, i_frame-1] += destination_prediction[:, i] * i_frame / self.future_len
