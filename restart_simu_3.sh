...
set_ros_parameters() {
    if [ ! -z "$human_detour" ]; then
        ...
        sed -i "s/human_detour: .*/human_detour: $human_detour/" $(rospack find ilq_solver)/config/3_2AgentDoorway_config.yaml
    fi

    if [ ! -z "$human_v" ]; then
        echo "Modifying config to set human_v to $human_v..."
        # Using sed to replace the parameter in the YAML file
        sed -i "s/human_v: .*/human_v: $human_v/" $(rospack find ilq_solver)/config/3_2AgentDoorway_config.yaml
    fi
}
...
relaunch_nodes() {
    ...
    roslaunch simu_3 restart.launch
}
